...
//
// Pack catalog: registry rules + detection-layer metadata, composed with the
// core catalog machinery (exact-then-wildcard lookup).

import { createRuleCatalog } from "@miaggy/core";
...
const catalog = createRuleCatalog({
  ...
  extraEntries: DETECTION_RULE_METADATA,
});

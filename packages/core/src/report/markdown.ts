...
import type { RuleCatalogEntry } from "../registry.js";
import { computePostureScore } from "../score.js";
...
/** Pack-supplied context: the data a pack injects into the shared renderer. */
export interface MarkdownReportContext {
  /** Title used when the caller passes none. */
  defaultTitle: string;
  /** Executive-summary category for a finding's ruleId. */
  categorize: (ruleId: string) => string;
  /** Metadata lookup for threat/rationale blocks. */
...
export function buildMarkdownReport(
  ...
  const title = opts.title ?? ctx.defaultTitle;
  ...
  for (const f of findings) {
    ...
    byStatus[f.status]++;
    const category = ctx.categorize(f.ruleId);
  ...
  if (criticals.length > 0) {
    ...
    for (const f of criticals) {
      ...
      const meta = ctx.getRuleMetadata(f.ruleId);
      if (meta) {
        lines.push("");
  ...
  lines.push("---");
  lines.push("");
  for (const line of ctx.footerLines) {
    lines.push(line);
  }

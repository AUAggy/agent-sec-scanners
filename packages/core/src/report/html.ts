...
/** Pack-supplied context: branding strings and metadata lookups the shared
 * renderer injects into the page. Every field is rendered verbatim (HTML-escaped). */
export interface HtmlReportContext {
  /** Title used when the caller passes none. */
  defaultTitle: string;
  /** Masthead eyebrow line, e.g. "Security assessment · bedrock-security-mcp". */
  eyebrow: string;
  /** Masthead scope line, e.g. "Assessed against OWASP LLM Top 10 (2025), …". */
  scopeLine: string;
  /** Footer left cell, e.g. "Generated by bedrock-security-mcp v0.1.0". */
  footerLeft: string;
  /** Footer right cell, e.g. the frameworks list. */
  footerRight: string;
  /** Human-readable labels for the framework tags the pack cites. Unknown tags render as-is. */
...
export function generateHtmlReport(
  ...
  const title = options.title ?? ctx.defaultTitle;
  const generatedAt = new Date().toISOString().replace("T", " ").slice(0, 19) + " UTC";

  const frameworkLabel = (tag: string): string => ctx.frameworkLabels[tag] ?? tag;
  ...
  let cardIndex = 0;
  function findingSection(severity: string, label: string, sectionFindings: Finding[]): string {
  ...
  function remediationTable(tableFindings: Finding[]): string {
  ...
  function complianceTable(tableFindings: Finding[]): string {
  ...
  function passedSection(passedFindings: Finding[]): string {
...
<footer class="report-footer">
  <span>${escapeHtml(ctx.footerLeft)}</span>
  <span>${escapeHtml(ctx.footerRight)}</span>
</footer>

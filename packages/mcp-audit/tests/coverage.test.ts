...
describe("coverageResidual", () => {
  it("returns null for a resolved npm server (it is assessed by the Wave-1 rules)", () => {
    const clean = entry({
      ...
      npmPackage: { spec: "well-kept@2.1.0", name: "well-kept", versionSpec: "2.1.0" },
    });
  ...
  it("names the residual per launch shape", () => {
    expect(coverageResidual(entry({ launchShape: "pypi", command: "uvx" }))).toMatch(/pypi/);
...
describe("coverageSkipFindings", () => {
  const clean = entry({
    ...
    npmPackage: { spec: "a@1.0.0", name: "a", versionSpec: "1.0.0" },
  });
